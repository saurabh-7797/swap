...
async function main() {
  try {
    console.log("🏊 POOL 2 - ADD LIQUIDITY");
    ...
    const liquidityRatio = amountA / amountB;
    console.log(`Liquidity Ratio: ${liquidityRatio.toFixed(6)} (A:B)`);
    ...
    // Calculate LP tokens gained
    const lpTokensGained = newBalanceLP - balanceLP;
    console.log(`\n🎯 LIQUIDITY ADDED SUCCESSFULLY!`);
    console.log(`LP Tokens Gained: ${formatTokenAmount(lpTokensGained)}`);
    console.log(`Total Value Locked: ${formatTokenAmount(amountA + amountB)} tokens`);
    console.log(`Pool 2 Type: Stable (0.01% fee)`);

  } catch (error) {
    console.error("❌ Error adding liquidity to Pool 2:", error.message);
    if (error.logs) {
